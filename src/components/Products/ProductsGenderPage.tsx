...
const ProductsGenderPage = ({ gender, title, type, name }: GenderProps) => {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      let newProducts
      if (type) {
        newProducts = await fetchProducts(page * 8, 8, {
          ...filter,
          gender,
          types: [type],
        })
    ...
    loadProducts()
  }, [filter, page, setTotalProducts, gender, type, name])
  ...
  return (
    <div className="mb-20 px-3">
      ...
      {!type && !name && (
        <FilterByTypeProduct propGender={gender} setPage={setPage} />
      )}
...
export default ProductsGenderPage
